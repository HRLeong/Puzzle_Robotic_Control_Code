...
class Dexarm:
    ...
    def move_to(self, position, feedrate=2000, mode="G0", wait=True):
        """ 
        ...
        Args:
            ...
            position (array/list): The position in list form (i.e.[X,Y,Z,E]), in millimeters by default. 
            Units may be set to inches by G20. Note that the center of y axis is 300mm.
            feedrate (int): set the feedrate for all subsequent moves
        """
        cmd = mode + "F" + str(feedrate)
        if position[0] is not None:
            cmd = cmd + "X"+str(round(position[0]))
        if position[1] is not None:
            cmd = cmd + "Y" + str(round(position[1]))
        if position[2] is not None:
            cmd = cmd + "Z" + str(round(position[2]))
        if position[3] is not None:
            cmd = cmd + "E" + str(round(position[3]))
        cmd = cmd + "\r\n"
        self._send_cmd(cmd, wait=wait)

    def fast_move_to(self, position, feedrate=2000, wait=True):
        """
        ...
        """
        self.move_to(self, position, feedrate, mode="G0", wait=wait)
